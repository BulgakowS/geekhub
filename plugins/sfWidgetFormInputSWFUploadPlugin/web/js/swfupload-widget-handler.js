...
swfu_widget.SWFUploadProgress.prototype.addObservers = function()
{
  ...
  swfu_widget.addObserver(swfu_widget.handlers, 'file_queued', function(event)
  {
    ...
    file.setMessage('Файл очереди');
    this._files_queued++;
    this._totalBytes += event.file.size;
    this.setMessage(this._files_queued+ ' файл(ов) в очереди');
    if(this.getIsError())
  ...
  swfu_widget.addObserver(this, 'begin_upload', function(event)
  {
    ...
    this.setMessage('Загрузка в процессе...');
  ...
  swfu_widget.addObserver(swfu_widget.handlers, 'queue_complete', function(event)
  {
    ...
    if(this.getIsError())
      this.setMessage('Ошибки во время загрузки.');
    else if(this.getIsCancelled())
      this.setMessage('Загрузка завершена. Несколько файлов были отменены во время загрузки.');
    else
...
swfu_widget.SWFUploadProgress.prototype.reset  = function()
{
  ...
  this.setMessage('Очередь пустая');
}
...
swfu_widget.SWFUploadProgressView.prototype.renderHTML = function()
{
  ...
  // Content
  this._html.cancelUploadLinkInner.innerHTML = 'Отменить загрузку';
  ...
  this._html.headline.innerHTML = 'Общий прогресс';
    ...
    if(event.file.id == this._file.id)
    {
      ...
      switch (event.error_code)
      {
        case SWFUpload.UPLOAD_ERROR.HTTP_ERROR :
          ...
          this.setMessage('Ошибка HTTP: ' + event.message + "&nbsp;" + swfu_widget.HTTP_STATUS[event.message]);
          break;
        case SWFUpload.UPLOAD_ERROR.MISSING_UPLOAD_URL :
          this.setMessage('Отсутствует адрес загрузки, отмена.');
          break;
        case SWFUpload.UPLOAD_ERROR.IO_ERROR :
          this.setMessage('Ошибка ввода/вывода');
          break;
        case SWFUpload.UPLOAD_ERROR.SECURITY_ERROR:
          this.setMessage('Ошибка безопасности');
          break;
        case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
          this.setMessage('Превышен максимальный размера файла.');
          break;
        case SWFUpload.UPLOAD_ERROR.UPLOAD_FAILED:
          this.setMessage('Сбой при загрузке.')
          break;
        case SWFUpload.UPLOAD_ERROR.SPECIFIED_FILE_ID_NOT_FOUND:
          this.setMessage('Файл не найден.');
          break;
        case SWFUpload.UPLOAD_ERROR.FILE_VALIDATION_FAILED:
          this.setMessage('Этот тип файла не допускается.')
          break;
        case SWFUpload.UPLOAD_ERROR.FILE_CANCELLED:
          this.setMessage('Загрузка отменена.')
          this.setIsCancelled(true);
          break;
        case SWFUpload.UPLOAD_ERROR.UPLOAD_STOPPED:
          this.setMessage('Загрузка прервана');
          this.setIsAborted(true);
          break;
      }
      this.getView().updateUI();
...
swfu_widget.SWFUploadProgressFileView.prototype.renderHTML = function()
{
  ...
  this._html.cancelUploadLink.innerHTML = '<span>Отмена</span>';
